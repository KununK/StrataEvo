...
轨迹验证假设。Evidence 按发生顺序保留工具名称、参数和返回结果，终态信号只描述观察到的
状态，不预先推断故障原因。四层归因用于形成假设和审计，不对文件实施严格的分层权限；同一
个模块可能同时包含上下文、工具和架构行为。结构化输出的原始尝试、解析错误及累计 Token
都会保存。

Diagnosis 请求使用独立的全局字符预算。预算内先保留异常 case 的短摘要，再按优先级加入
有序工具事件、错误和候选代码详情，避免 benchmark 规模或单次工具输出撑满模型上下文。
