...
def _compact_meta_messages(messages: list[Message], limit: int) -> list[Message]:
    ...
    while (
        len(groups) > 1
        and _messages_size([*prefix, *(message for group in groups for message in group)]) > limit
    ):
        groups.pop(0)
...
def _messages_size(messages: list[Message]) -> int:
    return sum(
        len(json.dumps(message.to_dict(), ensure_ascii=False, default=str)) for message in messages
    )
