...
但“可修改”不等于“可以使用修改后的组件给自己验收”。否则候选可以通过降低测试难度、
吞掉异常或改变分数计算获得虚假提升。
...
以 `evo_test_inter@b75e239` 运行完整 MBPP，结合已有 HumanEval 结果，确认代内反馈、晋级复测和
回滚流程在第二个 benchmark 上的实际表现。

### 阶段 1：全仓库候选工作区

不再直接在当前开发工作区修改候选。每代基于父代 commit 创建独立 Git worktree 或等价的候选
目录，允许修改全部受版本控制的项目文件。外部 Supervisor 保持父代代码，负责超时、回滚、
提交和运行记录。
...
1. 不继续根据小规模 MBPP smoke 结果修改诊断规则。
2. 在 `b75e239` 上完成一次全量 MBPP 实验。
3. 汇总 Diagnosis、Plan、实际 diff、加载时机和晋级结果。
4. 根据结果实现阶段 1 的隔离全仓库候选工作区。
